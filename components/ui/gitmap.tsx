...
export function Gitmap({
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const left = cellRect.left + cellRect.width / 2 - wrapperRect.left;
    const top = cellRect.top - wrapperRect.top;

    tooltipRef.current.style.left = `${left}px`;
    tooltipRef.current.style.top = `${top - 8}px`; // offset upward
          ...
          week.days.map((day, row) =>
            day === null ? (
              ...
              <div
                ...
                className={cn(
                  "rounded-[2px]",
                  showCounts && "flex items-center justify-center",
                )}
                style={{
                  gridRow: row + 1,
                  gridColumn: col + 1,
                  backgroundColor:
                    colors[LEVEL_COLORS[day.level as 0 | 1 | 2 | 3 | 4]],
                  width: cellSize,
                  height: cellSize,
                }}
                data-date={day.date}
                ...
                {showCounts && day.count > 0 && (
                  <span
                    className={cn(
                      ...
                      (day.level === 2 || day.level === 3) &&
                        "text-muted dark:text-foreground/80",
                      day.level === 4 && "text-background",
                    )}
                    style={{
                      fontSize: `${Math.max(8 - String(day.count).length, 4)}px`,
                    }}
                  >
                    {day.count}
                  </span>
